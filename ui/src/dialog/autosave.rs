//! Autosave-recovery modal — `Load autosave / Discard / Later`.
//! Mirrors `ui/src/dialog/confirm_close.rs` overlay pattern and panel colors.
//! Shown when `autosave_prompt()` is `Some`.
...
pub fn window<'a, S: UiState + ?Sized>(state: &S) -> Option<Element<'a, UiEvent>> {
    let prompt = state.autosave_prompt()?;
    let w = container(
        column![
            text(format!("Load autosave for \"{}\"?", prompt.title))
                .size(scale::s(14.0))
                .color(Color::WHITE),
            text(format!("Unsaved changes from {} were found.", prompt.saved_at_display))
                .size(scale::s(11.0))
                .color(MUTED_FG),
            text("Load restores the autosaved text and inpaint layers. Discard deletes the backup.")
                .size(scale::s(11.0))
                .color(MUTED_FG),
            ...
            row![
                ...
                button(text("Later").size(scale::s(12.0)))
                    ...
                    .on_press(UiEvent::AutosaveLater),
                button(text("Discard").size(scale::s(12.0)))
                    .padding([scale::s(6.0), scale::s(14.0)])
                    .style(danger_button_style)
                    .on_press(UiEvent::AutosaveDiscard(prompt.id)),
                button(text("Load autosave").size(scale::s(12.0)))
                    .padding([scale::s(6.0), scale::s(14.0)])
                    .style(accent_button_style)
                    .on_press(UiEvent::AutosaveLoad(prompt.id)),
            ]
            .spacing(scale::s(8.0))
...
/// Overlay that asks `Load autosave for {title}?` with `Load / Discard / Later`.
/// Returns `base` unchanged when `state.autosave_prompt()` is `None`.
