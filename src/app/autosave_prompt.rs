// Thin Message wrapper — widget construction lives in `easyscanlate-ui::dialog::autosave`.
...
pub fn view<'a>(app: &'a App, base: Element<'a, Message>) -> Element<'a, Message> {
    let Some(window) = easyscanlate_ui::dialog::autosave::window(app) else {
        return base;
    };
    ...
    .on_press(Message::Ui(easyscanlate_ui::event::UiEvent::AutosaveLater)));
    stack![
        base,
