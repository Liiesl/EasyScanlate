...
fn add_preset_fills_the_first_empty_slot() {
    let (mut app, _id) = app_with_entry();
    app.active_tab_mut().style_working.bg_color = [9, 9, 9, 255];
    let _ = update(&mut app, Message::Ui(UiEvent::StylePresetAdd));
    let _ = update(&mut app, Message::Ui(UiEvent::StylePresetAdd));
    let _ = update(&mut app, Message::Ui(UiEvent::StylePresetAdd));
    assert_eq!(app.presets.len(), INITIAL_PRESET_SLOTS);
    assert_eq!(app.presets.get(5), Some(app.active_tab().style_working.clone()));
    assert_eq!(app.presets.get(6), Some(app.active_tab().style_working.clone()));
    assert_eq!(app.presets.get(7), Some(app.active_tab().style_working.clone()));
}
...
fn add_preset_appends_when_all_slots_are_full() {
    ...
    assert_eq!(app.presets.get(INITIAL_PRESET_SLOTS), Some(app.active_tab().style_working.clone()));
}

#[test]
fn add_preset_refills_an_emptied_slot_before_appending() {
    let (mut app, _id) = app_with_entry();
    app.active_tab_mut().style_working.text_color = [7, 7, 7, 255];
    let _ = update(&mut app, Message::Ui(UiEvent::StylePresetRemove(2)));
    let _ = update(&mut app, Message::Ui(UiEvent::StylePresetAdd));
    assert_eq!(app.presets.len(), INITIAL_PRESET_SLOTS);
    assert_eq!(app.presets.get(2), Some(app.active_tab().style_working.clone()));
}

#[test]
fn replace_preset_overwrites_filled_and_empty_slots() {
    let (mut app, _id) = app_with_entry();
    app.active_tab_mut().style_working.text_color = [42, 0, 0, 255];
    let _ = update(&mut app, Message::Ui(UiEvent::StylePresetReplace(1)));
    assert_eq!(app.presets.get(1), Some(app.active_tab().style_working.clone()));
...
fn style_font_sets_family_and_loads_font() {
    ...
    assert_eq!(app.active_tab().style_working.font_family.as_deref(), Some("Test"));
    assert_eq!(
        app.active_tab().project.entry_style(id).font_family.as_deref(),
        Some("Test")
    );
}
...
fn style_text_align_sets_alignment() {
    ...
    assert_eq!(app.active_tab().style_working.text_align, TextAlign::Right);
    assert_eq!(
        app.active_tab().project.entry_style(id).text_align,
        TextAlign::Right
    );
}
